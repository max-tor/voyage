...
<template>
  <section class="mx-auto max-w-2xl px-4 py-6">
    <header class="mb-6 flex items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold tracking-tight">Fill-ups</h1>
        <p class="mt-0.5 text-sm text-slate-500">Your fuel log and consumption.</p>
      </div>
      <RouterLink
    ...
    >
      ...
      <p class="text-sm text-slate-500">Add a car first.</p>
      <RouterLink
        to="/garage/new"
    ...
    <template v-else>
      <label v-if="garage.cars.length > 1" class="mb-4 block">
        ...
        <select
          ...
          class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm shadow-soft focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30"
        >
      ...
      <p
        v-if="fillups.error"
        class="mb-4 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700"
      >
        {{ fillups.error }}
      </p>
      ...
      <div
        ...
        class="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-white/60 px-6 py-16 text-center"
      >
        <div class="grid h-12 w-12 place-items-center rounded-2xl bg-brand-50 text-brand-600">
          <Fuel :size="22" />
        </div>
        <p class="text-sm text-slate-500">No fill-ups yet for this car.</p>
      </div>
      ...
      <ul v-else class="space-y-3">
        <li
          ...
          :key="f.id"
          class="rounded-2xl bg-white p-3.5 shadow-soft ring-1 ring-slate-900/5 transition-shadow hover:shadow-soft-md"
        >
          <RouterLink :to="`/fillups/${f.id}`" class="flex items-center gap-3">
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                <p class="text-sm font-semibold text-slate-900">{{ formatDate(f.date) }}</p>
              ...
              <p
                ...
                class="mt-1 text-xs font-semibold text-brand-600"
              >
